...
class Router
{
    ...
    {
        // Get cached APIs
        $transient_name = 'bricksbender_scanned_apis_' . BRICKSBENDER::VERSION;
         ...
         */
        \do_action('a!yabe/bricksbender/api/router:before_scan', $finder);
        ...
        foreach ($declared_classes as $declared_class) {
            ...
            if (!$reflector->isSubclassOf(\Yabe\Bricksbender\Api\ApiInterface::class)) {
                continue;
            }
    ...
    public function register_apis() : void
    {
        ...
        $apis = \apply_filters('f!yabe/bricksbender/api/router:register_apis', $this->apis);
        foreach ($apis as $api) {
