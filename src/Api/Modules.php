 ...
 * @since 2.0.0
 */
class Modules extends \Yabe\Bricksbender\Api\AbstractApi implements \Yabe\Bricksbender\Api\ApiInterface
{
    ...
    public function index(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        foreach ($modules as $module) {
            ...
            $items[] = ['id' => $module['name'], 'title' => $moduleInstance->get_title(), 'description' => $moduleInstance->get_description(), 'icon' => $moduleInstance->get_icon(), 'status' => $moduleInstance->is_enabled(), 'version' => $moduleInstance->get_version(), 'hasSettingPage' => $moduleInstance->has_setting_page()];
        }
        return new WP_REST_Response(['data' => ['modules' => $items]]);
    }
    private function update_status(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        if ($module === null) {
            return new WP_REST_Response(['message' => \__('Module not found', 'yabe-bricksbender')], 404, []);
        }
        /** @var ModuleInterface $moduleInstance */
        $moduleInstance = $module['instanceWithoutConstructor'];
