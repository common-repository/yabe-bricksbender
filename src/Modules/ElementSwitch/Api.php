...
class Api extends AbstractApi implements ApiInterface
{
    ...
    public function index(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        foreach (Elements::$elements as $e) {
            ...
            $elements[] = ['namespace' => 'bricks', 'id' => $e['name'], 'name' => $eDetails['label'] ?: $e['name'], 'category' => $eDetails['category'], 'icon' => $eDetails['icon']];
        }
        foreach (ElementsLoader::get_instance()->get_elements() as $e) {
            $elements[] = ['namespace' => 'bricksbender', 'id' => $e['name'], 'name' => $e['label'], 'category' => $e['instanceWithoutConstructor']->category, 'icon' => $e['instanceWithoutConstructor']->icon];
        }
        // loop to add the enabled status
        foreach ($elements as $key => $element) {
            $elements[$key]['status'] = (bool) Config::get(\sprintf('elements.%s.%s.enabled', $element['namespace'], $element['id']), \true);
        }
        return new WP_REST_Response(['data' => ['elements' => $elements]]);
    }
