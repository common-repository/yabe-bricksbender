...
class Loader
{
    ...
    public function scan_elements()
    {
        ...
        foreach ($declared_classes as $declared_class) {
            ...
            if (!$reflector->isSubclassOf(\Yabe\Bricksbender\Elements\ElementInterface::class)) {
                continue;
            }
            // Get Element detail and push to Loader::$elements to be register later
            /** @var ElementInterface $element */
            $element = $reflector->newInstanceWithoutConstructor();
            $this->elements[$element->get_identifier()] = ['name' => $element->get_identifier(), 'file_path' => $reflector->getFileName(), 'class_name' => $reflector->getName(), 'label' => $element->get_label(), 'instanceWithoutConstructor' => $element];
        }
    ...
    public function register_elements() : void
    {
        ...
        foreach ($elements as $element) {
            ...
            if (!Config::get(\sprintf('elements.bricksbender.%s.enabled', $element['name']), \true)) {
                continue;
            }
            BricksElements::register_element($element['file_path'], $element['name'], $element['class_name']);
        }
    }
    /**
